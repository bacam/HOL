...
fun GEN_COND_CASES_TAC P (asl, w) =
   let
      ...
      fun mksubs th c = CONV_RULE (LHS_CONV (RATOR_CONV (RATOR_CONV (RAND_CONV (K (SYM th)))))) c
   in
      DISJ_CASES_THEN2
        (fn th =>
           SUBST1_TAC (mksubs (EQT_INTRO th) ct) THEN ASSUME_TAC th)
        (fn th =>
           SUBST1_TAC (mksubs (EQF_INTRO th) cf) THEN ASSUME_TAC th)
